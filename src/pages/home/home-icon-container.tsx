  ...
  return (
    <div className="grid sm:flex grid-cols-3 gap-1 sm: sm:grid-cols-none sm:center w-full ">
      <HomeIcon
        link="/transfer"
        color="green"
        icon={<FcMoneyTransfer className="w-full absolute z-10 h-full" />}
        name="Transfer"
      />
      <HomeIcon
        link="/request-loan"
        color="red"
        icon={<FcMultipleDevices className="w-full absolute z-10 h-full" />}
        active={loan === null}
      ...
      {user && !user.is_approved && (
        <HomeIcon
          ...
          color="blue"
          icon={<FcPortraitMode className="w-full absolute z-10 h-full" />}
          name="Validasi Akun"
        />
      )}
    </div>
